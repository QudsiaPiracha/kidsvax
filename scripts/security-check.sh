...
# Check for EXPO_PUBLIC_* secrets (common mistake)
if [ -n "$STAGED_FILES" ]; then
  if echo "$STAGED_FILES" | xargs grep -l -E 'EXPO_PUBLIC_.*SECRET|EXPO_PUBLIC_.*SERVICE_ROLE' 2>/dev/null; then
    echo "ERROR: Secrets in EXPO_PUBLIC_* env vars are exposed to client!"
    exit 1
  fi
fi
